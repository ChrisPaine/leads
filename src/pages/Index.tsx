...
const Index = () => {
  ...
  const handleExampleSelect = (exampleType: 'business-leads' | 'pain-points' | 'real-estate' | 'free-furniture' | 'nanny-jobs') => {
    if (exampleType === 'business-leads') {
      setTimeFilter('day');
      setMainTopic('maine');
      setCommentsSearch('carpenter, carpentry, trim work, framing');
      setContactsSearch('need, needed, looking for, recommend, hiring, hire, quote');
      setExclusionsSearch('seeking work, I am a, available for, offering services');
      setSelectedPlatforms(['facebook']);
      setSelectedPhrases([]);
    } else if (exampleType === 'pain-points') {
      setTimeFilter('any');
      setMainTopic('e bikes owner');
      setCommentsSearch('');
      setContactsSearch('');
      setExclusionsSearch('');
      setSelectedPlatforms(['facebook', 'reddit']);
      handleSelectAllPhrases();
    } else if (exampleType === 'real-estate') {
      setTimeFilter('year');
      setMainTopic('maine');
      setCommentsSearch('sq, psf, per sqft, square foot, per square, sf');
      setContactsSearch('build, building, built, construction, new build, cost to build, building costs, estimate, quote, recently built, what did you pay, ended up costing');
      setExclusionsSearch('for rent, renting, lease, tenant, landlord, monthly rent, deposit, available now, bedrooms bath, listing, MLS, realtor, buying, sold, foreclosure, fixer upper, renovated, updated kitchen, basement apartment, Airbnb');
      setSelectedPlatforms(['facebook', 'reddit']);
      setSelectedPhrases([]);
    } else if (exampleType === 'free-furniture') {
      setTimeFilter('day');
      setMainTopic('maine');
      setCommentsSearch('table');
      setContactsSearch('need, needed, for sale, free');
      setExclusionsSearch('');
      setSelectedPlatforms(['facebook']);
      setSelectedPhrases([]);
    } else if (exampleType === 'nanny-jobs') {
      setTimeFilter('day');
      setMainTopic('maine');
      setCommentsSearch('nanny');
      setContactsSearch('watch, need, needed, looking for, recommend, hiring, hire');
      setExclusionsSearch('');
      setSelectedPlatforms(['facebook']);
      setSelectedPhrases([]);
    }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (formattedComments && formattedContacts) {
        ...
        queryString += ` ${formattedComments}`;
      } else if (formattedContacts) {
        queryString += ` ${formattedContacts}`;
      }
                ...
                <CardContent className="pt-3 pb-3 px-4">
                  ...
                  <div className="space-y-3">
                    {getSortedResults().map(result => {
                      ...
                      const keywords = [
                        mainTopic,
                        ...commentsSearch.split(',').map(k => k.trim()),
                        ...contactsSearch.split(',').map(k => k.trim()),
                        ...selectedPhrases,
                      ].filter(k => k.length > 0);
