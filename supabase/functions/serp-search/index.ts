...
serve(async (req) => {
  ...
  try {
    ...
    const searchPromises = platforms.map(async (platform) => {
      ...
      // Format the query to add OR operators between quoted terms
      // Pattern: ("term1" "term2" "term3") becomes ("term1" OR "term2" OR "term3")
      let formattedQuery = query;

      // First, clean up any escaped quotes or malformed quotes
      formattedQuery = formattedQuery.replace(/\\"/g, '"');
      formattedQuery = formattedQuery.replace(/\"\s*\"/g, '" "'); // Fix double quotes with no space

      // Replace patterns like ("word1" "word2" "word3") with ("word1" OR "word2" OR "word3")
      // Updated regex to handle any characters inside quotes, not just word characters
      formattedQuery = formattedQuery.replace(/\(("[^"]*"(?:\s+"[^"]*")+)\)/g, (_match, group) => {
        const withOr = group.replace(/"\s+"/g, '" OR "');
        return `(${withOr})`;
      });

      if (formattedQuery !== query) {
        console.log(`Query transformed from: ${query}`)
        console.log(`Query transformed to: ${formattedQuery}`)
      }

      // Provider exclusions - filter out contractors/carpenters advertising their services
      // Based on actual results analysis - these patterns indicate provider advertisements
      const providerExclusions = [
        '-"seeking immediate work"',
        '-"seeking work"',
        '-"carpenter seeking"',
        '-"looking for work"',
      ...
      // Add user's custom exclusions if provided
      // Parse custom exclusions and add minus signs if not already present
      const userExclusions = customExclusions.trim() ?
        customExclusions.split(/\s+/).map(term => {
          // If term is already quoted and doesn't start with -, add -
          if (term.startsWith('"') && !term.startsWith('-')) {
            return `-${term}`;
          }
          // If term is not quoted, quote it and add -
          if (!term.startsWith('-') && !term.startsWith('"')) {
            return `-"${term}"`;
          }
          return term;
      ...
      const allExclusions = [providerExclusions, userExclusions].filter(Boolean).join(' ');

      const searchQuery = `${siteFilter} ${formattedQuery} ${allExclusions}`;
    ...
    // Title-based provider filters - these patterns in titles indicate provider advertisements
    const titleProviderPatterns = [
      /currently taking on work/i,
      /currently taking on/i,
      /taking on work/i,
      /\w+ services for/i,  // "handyman services for", "carpentry services for", etc.
      /handyman services/i,
      /carpentry services/i,
      /carpenter services/i,
      /construction services/i,
      /we offer/i,
      /services in \w+/i,  // "services in [city]"
    ];

    // Filter out provider advertisements based on title content
    const customerResults = allResults.filter(result => {
      const titleLower = result.title.toLowerCase();

      // Check if title matches any provider patterns
      for (const pattern of titleProviderPatterns) {
        if (pattern.test(titleLower)) {
          console.log(`🚫 PROVIDER FILTERED (title): "${result.title}"`)
          return false;
        }
      }
    ...
    console.log(`Filtered out ${allResults.length - customerResults.length} provider ads based on title patterns`)

    // Deduplicate results by URL and title (keep first occurrence)
    const seenUrls = new Set<string>()
    const seenTitles = new Set<string>()
    const deduplicatedResults = customerResults.filter(result => {
